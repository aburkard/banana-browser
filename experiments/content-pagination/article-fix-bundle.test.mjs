...
const readable=process.env.BANANA_READABLE_TEST==='1';
let bundle;try{bundle=await readFile(new URL(`../../public/tmp/${readable?'readable-article-check':'article-fix-check'}/article-fix.js`,import.meta.url),'utf8');}catch{}
test('actual article source selection and production scroll run twice with a mocked provider',{skip:!bundle},async()=>{
 ...
 const report=JSON.parse(nodes.get('#report').textContent);assert.equal(report.variant,readable?'readable-source':'html-source');assert.equal(report.state,'completed');assert.equal(report.images,2);assert.equal(images,2);assert.equal(report.scroll,1);assert.ok(report.checks.every(check=>check.matches));assert.ok(report.checks.some(check=>check.scrollPrompt));assert.doesNotMatch(nodes.get('#report').textContent,/fake-gemini|aW1hZ2U=/);
