...
const target=new URL('../../public/tmp/advancing-article-check/',import.meta.url);
if(process.argv.includes('--clean')){await rm(target,{recursive:true,force:true});console.log('Removed temporary article-fix check.');}
 ...
 const result=await build({entryPoints:[fileURLToPath(new URL('./advancing.mjs',import.meta.url))],bundle:true,format:'iife',platform:'browser',minify:true,write:false,define:{__ARTICLE_FIXTURE__:JSON.stringify(fixture)}});
 ...
 console.log('http://127.0.0.1:5178/banana-browser/tmp/advancing-article-check/index.html');console.log('Run --clean before production build.');
}
