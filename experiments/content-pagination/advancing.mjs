...
const fixture=__ARTICLE_FIXTURE__,lock='banana-advancing-article-v1';
...
const windows=originalSource?advancingSources(readableSource(originalSource)):[];
if(windows.length>3)throw new Error('Fixture exceeds three-image experiment cap');
let cursor=0,source=windows[0];
if(source)sections[index]=source;
const report={variant:'advancing-source',originalSourceChars:originalSource?.length,windowCount:windows.length,state:'ready',images:0,references:0,stopped:false,section:index,sectionCount:sections.length,teamHeadings:originalSource?headings(originalSource):[],fixtureHash:fixture.sha256,checks:[],usage:[],timings:[]};
...
async function step(name,work){
 ...
 try{await work();if(app.state.error)throw new Error(app.state.error);await display();report.state=cursor===windows.length-1?'completed':'awaiting-scroll';$('#status').textContent=`Passage ${cursor+1}/${windows.length} · ${report.images}/3 image calls · ${report.state}`;}
...
$('#generate').onclick=async()=>{
 ...
 globalThis.fetch=articleGuard({nativeFetch,state:report,persist,referenceHosts:fixture.referenceHosts,maxImages:3});
...
$('#save-json').onclick=()=>{const text=$('#report').textContent;if(!text)return;const url=URL.createObjectURL(new Blob([text],{type:'application/json'}));download('advancing-results.json',url);setTimeout(()=>URL.revokeObjectURL(url),1000);};
$('#save-image').onclick=()=>{if(app?.state.currentImage)download(`advancing-passage-${cursor}.png`,canvas.toDataURL('image/png'));};
