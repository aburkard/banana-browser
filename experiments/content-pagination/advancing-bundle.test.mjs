...
let bundle;try{bundle=await readFile(new URL('../../public/tmp/advancing-article-check/advancing.js',import.meta.url),'utf8');}catch{}
test('source advances for three images, stops at exhaustion and cached navigation makes no requests',{skip:!bundle},async()=>{
 ...
 assert.match(nodes.get('#headings').textContent,/Jacksonville/);await nodes.get('#generate').onclick();await nodes.get('#scroll').onclick();await nodes.get('#scroll').onclick();await nodes.get('#scroll').onclick();await nodes.get('#history').onclick();await nodes.get('#generate').onclick();
 const report=JSON.parse(nodes.get('#report').textContent);assert.equal(report.variant,'advancing-source');assert.equal(report.state,'completed');assert.equal(report.images,3);assert.equal(images,3);assert.equal(report.scroll,2);assert.ok(report.checks.every(check=>check.matches));assert.ok(report.checks.some(check=>check.scrollPrompt));assert.doesNotMatch(nodes.get('#report').textContent,/fake-gemini|aW1hZ2U=/);
 assert.deepEqual(report.checks.filter(check=>check.kind==='input-source').map(check=>check.window.hasMore),[true,true,false]);
