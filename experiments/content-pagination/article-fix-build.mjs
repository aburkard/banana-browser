...
const target=new URL(`../../public/tmp/${readable?'readable-article-check':'article-fix-check'}/`,import.meta.url);
if(process.argv.includes('--clean')){await rm(target,{recursive:true,force:true});console.log('Removed temporary article-fix check.');}
 ...
 const result=await build({entryPoints:[fileURLToPath(new URL('./article-fix.mjs',import.meta.url))],bundle:true,format:'iife',platform:'browser',minify:true,write:false,define:{__ARTICLE_FIXTURE__:JSON.stringify(fixture),__READABLE_SOURCE__:String(readable)}});
 ...
 console.log(`http://127.0.0.1:5178/banana-browser/tmp/${readable?'readable-article-check':'article-fix-check'}/index.html`);console.log('Run --clean with the same variant flag before production build.');
}
